 ...
 * It submits the task asynchronously and blocks with wait() until the task finishes. for more
 * information, please refer to https://help.aliyun.com/document_detail/2712535.html
 */
public class RecognizeSpeechTransWait {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // asyncCall submits the task and returns immediately
      TranscriptionResult submitResult = transcription.asyncCall(param);
      String taskId = submitResult.getTaskId();
      if (taskId == null) {
        System.err.println("task submission failed: " + submitResult.getOutput());
        System.exit(1);
      }
      System.out.println("task submitted, requestId: " + submitResult.getRequestId());
      System.out.println("taskId: " + taskId);

      // wait() blocks until the task status becomes SUCCEEDED or FAILED
